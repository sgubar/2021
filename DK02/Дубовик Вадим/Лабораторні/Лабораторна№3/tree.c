...
void mergeTrees(Tree* aTreeDst, Tree* aTreeSrc)
{
	while (NULL != aTreeDst)
	{
		//ToBegining
		Node* tmp = aTreeDst->root;
		//Root is deleted
		if (NULL == tmp)
		{
			free(aTreeDst);
		...
		//Insert from tree to tree

		insertValueToTree(aTreeSrc, tmp->value);
		deleteNodeWithValue(aTreeDst,tmp->value);
	}
	return;
}
