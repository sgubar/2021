...
void deleteNodeWithValue(IntTree* aTree, int aValue) {
	...
	if (NULL == current->leftChild && NULL == current->rightChild) { // якщо вузол не маЇ нащадк≥в
	...
	else if (NULL == current->rightChild) { //€кщо вузол маЇ л≥вого нащадка
		if (aTree->root == current) {
	...
	else if (NULL == current->leftChild) { // €кщо вузол маЇ правого нащадка
		if (aTree->root == current) {
	...
	else {    //якщо вузол маЇ двох нащадк≥в
