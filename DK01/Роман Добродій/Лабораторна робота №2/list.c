...
int countAverageFromList(IntList* list)
{
	...
	int result = 0;
	IntListElement* tmp = list->head;
	while (tmp != NULL)
	{
		result=result + tmp->value;
		tmp = tmp->next;
	}
	result = result / countOfIntList(list);
	return result;
}
void deleteLessIndexFromList(IntList* list, int index)
{
	...
	int elementNumber = 0;
	IntListElement* tmp = list->head;
	for (; tmp->value > index; tmp = tmp->next, elementNumber++);
	...
	removeIntElementFromIntListAtIndex(list, elementNumber);
	return;
}
