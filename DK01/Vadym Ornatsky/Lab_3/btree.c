...
static void PrintNodePreorder(T_Node* node) {
	if (NULL == node) {
		return;
	}
    printf("%d ", node->value);
...
static void PrintNodeSymmetric(T_Node* node) {
	if (NULL == node) {
		return;
	}
    PrintNodeSymmetric(node->left);
    printf("%d ", node->value);
    PrintNodeSymmetric(node->right);
}

static void PrintNodePostorder(T_Node* node) {
	if (NULL == node) {
		return;
	}
...
void PrintTree(T_Tree* tree_, E_PrintOrder printOrder_) {
    ...
    switch (printOrder_) {
        case e_Preorder:
            PrintNodePreorder(tree_->root);
            break;
        case e_Symmetric:
            PrintNodeSymmetric(tree_->root);
            break;
        case e_Postorder:
            PrintNodePostorder(tree_->root);
            break;
    }
    ...
    while (traverseNode) {
        ...
        if (value_ > traverseNode->value) {
            ...
            if (NULL == traverseNode) {
                ...
                printf("Successfully inserted node with value: %d at right from %d\n", value_, parentNode->value);
                break;
            }
            ...
            if (NULL == traverseNode) {
                ...
                printf("Successfully inserted node with value: %d at left from %d\n", value_, parentNode->value);
                break;
            }
        } else {
...
void DeleteNode(T_Tree* tree_, char value_) {
	...
	tree_->count--;
    printf("Node with value: %d - successfully removed from tree.\n", value_);
    return;
}
