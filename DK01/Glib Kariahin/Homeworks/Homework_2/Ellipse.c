...
Ellipse *createEllipseByFocuses(Point* f1_,Point* f2_,int a,int b) {
    if (NULL == f1_ || NULL == f2_) {
        return NULL;
    }
    Ellipse *ellipse = (Ellipse *) malloc(sizeof(Ellipse));
    if (NULL!=ellipse) {
    if (ellipseValidation(a, b, f1_->x, f2_->x) == 1) {
...
Ellipse *createEllipseByCoords(int x1, int x2, int a,int b) {
    Ellipse *ellipse = (Ellipse *)malloc(sizeof(Ellipse));
    if(NULL != ellipse){
    if (ellipseValidation(a,b,x1,x2)==1) {
...
Ellipse *copyEllipse(Ellipse *ellipse) {
    ...
    return createEllipseByFocuses(ellipse->f1, ellipse->f2,ellipse->a,ellipse->b);
}
