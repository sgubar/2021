...
long countOfElementforMintoMax(IntList *list){
  if (NULL == list || list->head ==NULL) {
	...
	for (IntListElement *element = MinListElement(list); element !=max ; element = element->next, count ++){
...
IntList *SortForMinToMax(IntList *list){
    ...
    if (countOfElementforMintoMax(list) == 0){
        return 0;
    }
     IntListElement *element = NULL;
     IntListElement *max = MaxListElement(list);
     for(int i=0; i<countOfElementforMintoMax(list);i++){
  for(element = MinListElement(list); element !=max; element = element->next){
        int p =element->value ;
